  ...
  <table class="table table-hover"> 
   ...
   <tbody> 
    ...
    <tr> 
     ...
     <td><a 
      href="./CartDetailsServlet?add=1&uid=<%=userName%>&pid=<%=product.getProdId()%>&avail=<%=product.getProdQuantity()%>&qty=<%=prodQuantity%>"><i 
       class="fa fa-plus"></i></a></td> 
     <td><a 
      href="./CartDetailsServlet?add=0&uid=<%=userName%>&pid=<%=product.getProdId()%>&avail=<%=product.getProdQuantity()%>&qty=<%=prodQuantity%>"><i 
    ...
    <tr style="background-color: grey; color: white;"> 
     ...
     <td colspan="2" align="center"><form method="post"> 
       <button style="background-color: blue; color: white;" 
        formaction="payment.jsp?amount=<%=total%>">Pay Now</button> 
      </form></td> 
